...
export function buildXAxis(ds: Dataset): XAxis {
  ...
  if (ds.time && ds.time.length) {
    return { x: ds.time.slice(0, n), label: "Time (s)", isTime: true };
  }
  ...
  if (dtMs) {
    ...
    for (let i = 0; i < n; i++) x[i] = (i * dtMs) / 1000;
    return { x, label: "Time (s)", isTime: true };
  }

  const x = new Array<number>(n);
  for (let i = 0; i < n; i++) x[i] = i;
  return { x, label: "Sample #", isTime: false };
}
...
export function durationLabel(ds: Dataset): string {
  ...
  if (!ax.isTime) return `${n} samples`;
  const seconds = ax.x[n - 1] - ax.x[0];
  if (!Number.isFinite(seconds) || seconds <= 0) return `${n} samples`;
